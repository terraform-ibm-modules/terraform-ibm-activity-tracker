...
module "kms" {
  ...
  count                       = (local.use_kms_module && (length(coalesce(local.buckets_config, [])) != 0)) ? 1 : 0 # no need to create any KMS resources if kms_encryption_enabled_buckets is false or buckets_config length is 0
